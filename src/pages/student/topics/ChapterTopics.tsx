...
import { Chapter, ChapterTopic } from "@/types/chapter.types";

const ChapterTopics: React.FC = () => {
    ...
    useEffect(() => {
        const fetchChapterTopics = async () => {
            if (!chapterId || !userId) return;

            try {
                const response = await TopicService.getChapterTopics(chapterId, userId);
    ...
    const isTopicLocked = (topic: ChapterTopic) => {
        if (parsedUser?.schoolCategory !== "government") {
            return false;
        }
        if (!topic.unlocksAt) {
            return false;
    ...
    const renderTopicCard = (topic: ChapterTopic) => {
        ...
        return (
            ...
            >
                <div className="relative mx-4 mt-4 rounded-[1.5rem] overflow-hidden aspect-[1.2/1]">
                    <img
                        src={topic.attachmentUrl || "/placeholder-topic.png"}
                        alt={topic.topicName}
                        className={`h-full w-full object-cover transition duration-300 ${locked ? "filter grayscale" : ""
                            }`}
                    />
                    ...
                    <Link
                        ...
                        className={`absolute bottom-3 right-3 ${locked ? "pointer-events-none" : ""
                            }`}
                    >
                        <Button
                            size="sm"
                ...
                <div className="flex-grow px-5 py-4">
                    ...
                    <p className="text-sm text-gray-500 mt-1">{topic.cefrLevel}</p>
                </div>
            </Card>
        );
    };
            ...
            <div className="mx-auto px-4 py-6">
                <div className="flex items-center mb-6">
                    <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate("/student/learning-modes/curriculum-mode")}
                        className="mr-2"
                    >
                ...
                {topics.length > 0 ? (
                    ...
                    <div className="col-span-full text-center py-10">
                        <p className="text-muted-foreground">No topics available in this chapter</p>
                    </div>
                )}
            </div>
        </>
