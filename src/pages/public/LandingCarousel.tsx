...
export default function LandingCarousel() {
    ...
    return (
        <div className="relative top-20 w-full max-w-7xl mx-auto px-4 py-8 md:py-12 font-sans mb-20" dir="ltr">
            ...
            {/* Dots (Bottom Center) */}
            <div className="flex justify-center items-center gap-2 mt-8 sm:mt-12 mb-10">
                {[0, 1, 2].map((index) => (
                    <button
                        key={index}
...
function HeroSlide({ onStart, language }: { onStart: () => void; language: "ar" | "en" }) {
    ...
    return (
        <div className={cn("flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 py-6 sm:py-10")} dir={direction}>
            {/* Text Content */}
            <div className={cn(
                "w-full md:w-1/2 flex flex-col px-2 sm:px-0",
                isArabic ? "items-start text-right" : "items-start text-left",
                "order-1"
            )}>
                {!isArabic && (
                    <div className="inline-flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-1.5 sm:py-2 rounded-full border border-[#F0F0F0] shadow-[0_2px_10px_rgba(0,0,0,0.03)] mb-4 sm:mb-6 lg:mb-8 bg-white">
                ...
                {isArabic && (
                    <div className="inline-flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-1.5 sm:py-2 rounded-full border border-[#F0F0F0] shadow-[0_2px_10px_rgba(0,0,0,0.03)] mb-4 sm:mb-6 lg:mb-8 bg-white">
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2">
                            <span className="text-[12px] sm:text-[14px] md:text-[16px] lg:text-[18px] font-bold text-gray-700 leading-none">مدعوم بواسطة</span>
                            <span className="text-[12px] sm:text-[14px] md:text-[16px] lg:text-[18px] font-black text-[#F6AD55] leading-none">Zayd AI</span>
                        </div>
                    </div>
                )}

                <h1 className="text-[20px] sm:text-[28px] md:text-[40px] lg:text-[56px] xl:text-[64px] font-[800] text-gray-900 leading-[1.3] sm:leading-[1.2] md:leading-[1.1] mb-3 sm:mb-4 md:mb-6">
                    {isArabic ? (
                        <>
                            <span className="block">ممتعة مع تعلّم اللغة الصينية بطريقة</span>
                ...
                <p className="text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px] text-gray-600 mb-6 sm:mb-8 md:mb-10 max-w-lg leading-relaxed font-medium">
                    {isArabic
...
function FeaturesSlide({ language }: { language: "ar" | "en" }) {
    ...
    return (
        <div className="flex flex-col items-center py-6 sm:py-10" dir={direction}>
            <h2 className="text-[20px] sm:text-[28px] md:text-[36px] lg:text-[48px] font-[800] text-gray-900 text-center mb-3 sm:mb-4 md:mb-6 leading-tight px-4">
                {isArabic ? (
                    <>
                        <span className="block">تعلم اللغة الصينية مثلما</span>
                        <span className="block hidden sm:block">تفعل في الحياة الواقعية</span>
                        <span className="block sm:hidden">تفعل في الحياة الواقعية</span>
                    </>
                ) : (
                    <>
            ...
            <p className="text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px] text-gray-600 text-center max-w-4xl mb-6 sm:mb-8 md:mb-12 lg:mb-16 leading-relaxed font-medium px-4">
                {isArabic ? (
                    <>
                        <span className="block">تدرب على محادثات الحياة اليومية مثل التحيات، والطعام، والسفر، والمدرسة</span>
                        <span className="block hidden sm:block mt-2">يستمع الذكاء الاصطناعي إليك، ويصححك فورًا، ويساعدك على التحدث بشكل أكثر طبيعية — خطوة بخطوة</span>
                        <span className="block sm:hidden mt-2">يستمع الذكاء الاصطناعي إليك، ويصححك فورًا، ويساعدك على التحدث بشكل أكثر طبيعية</span>
                    </>
                ) : (
                    <>
    ...
    return (
        <div className={cn("flex flex-col md:flex-row items-center justify-between gap-6 sm:gap-8 md:gap-12 py-6 sm:py-10")} dir={direction}>
            ...
            <div className={cn(
                ...
                <h2 className="text-[20px] sm:text-[28px] md:text-[40px] lg:text-[52px] font-[800] text-gray-900 leading-[1.3] sm:leading-[1.2] md:leading-[1.1] mb-3 sm:mb-4 md:mb-6">
                    {isArabic ? (
                        <>
                            <span className="block">تعلم يوميًا</span>
                            <span className="text-[#FF5C5C] block">تقدم في المستوى بشكل أسرع</span>
                        </>
                    ) : (
                        <>
                ...
                <p className="text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px] text-gray-600 mb-6 sm:mb-8 md:mb-12 leading-relaxed font-medium">
                    {isArabic ? (
                        <>
                            <span className="block">تتبع سلاسل تعلمك وشاهد تحسنك مع مرور الوقت</span>
                            <span className="block hidden sm:block mt-2">افتح الإنجازات، واحصل على الشارات، وتنافس مع الأصدقاء لتبقى متحفزًا ومواظبًا</span>
                            <span className="block sm:hidden mt-2">افتح الإنجازات، واحصل على الشارات، وتنافس مع الأصدقاء</span>
                        </>
                    ) : (
                        <>
            ...
            {/* Visual Mockups */}
            <div className="w-full md:w-[60%] relative flex justify-center order-2 mt-4 md:mt-0">
