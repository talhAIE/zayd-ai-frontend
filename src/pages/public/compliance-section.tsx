...
const cards = [
  {
    ...
    title: "Zero Judgment",
    arabicTitle: "عدم الحكم على الآخرين",
    description:
      "The gym is open 24/7. Practice as much as you want, wherever you are.",
    arabicDescription:
      "الجم gym مفتوح على مدار الساعة طوال الأسبوع. تدرب كما تريد، أينما كنت.",
    rotation: -6,
    shift: "-translateX(15%)",
  },
  {
  ...
  },
  {
    ...
    title: "Local Roots",
    arabicTitle: "الجذور المحلية",
    description:
      "AI that actually understands the Saudi accent and provides better corrections.",
    arabicDescription: "ذكاء اصطناعي يفهم اللهجة السعودية ويقدم تصحيحات أفضل.",
    rotation: 6,
    shift: "translateX(15%)",
  },
];
    ...
    >
      <div className="max-w-7xl mx-auto px-4">
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, #76ABF8 0%, #058BF4 48.56%, #63B3F6 80%)",
                  }}
                >
                  <div
                    className={
                      isDesktop && card.isCenter ? "scale-125" : "scale-90"
                    }
                  >
                    {card.icon}
                  </div>
                ...
                <div
                  className={`${isDesktop && card.isCenter ? "space-y-4" : "space-y-2"}`}
                >
                  <h3
                    className={`${isDesktop && card.isCenter ? "text-3xl" : "text-2xl"} font-extrabold text-[#111827] tracking-tight`}
                  >
                    {isAr ? card.arabicTitle : card.title}
                  </h3>
                  <p
                    className={`${isDesktop && card.isCenter ? "text-lg" : "text-base"} text-gray-500 font-medium leading-relaxed`}
                  >
        ...
        <div className="flex justify-center">
          <Link to="/login">
            <button className="group px-10 py-5 rounded-full bg-gradient-to-r from-[#76ABF8] via-[#058BF4] to-[#63B3F6] text-white font-extrabold text-lg md:text-xl shadow-[0_20px_40px_-10px_rgba(5,139,244,0.3)] flex items-center gap-3 active:scale-95 transition-all duration-300">
              {isAr ? "تعلم مع زيد من اليوم" : "Learn With Zayd From Today"}
              <ChevronRight
                className={`w-6 h-6 transition-transform duration-300 ${isAr ? "rotate-180 group-hover:-translate-x-1" : "group-hover:translate-x-1"}`}
              />
            </button>
          </Link>
        </div>
