...
const features = [
  {
    ...
    arabicTitle: "ابق مسؤولاً",
  },
  {
    emoji: "💪",
    title: "Build the Habit",
    arabicTitle: "بناء العادة",
    description: "Track your weekly 60-minute goal on your dashboard.",
    arabicDescription:
      "تتبع هدفك الأسبوعي البالغ 60 دقيقة على لوحة التحكم الخاصة بك.",
  },
  {
    emoji: "📈",
    title: "See the Gains",
    arabicTitle: "شاهد المكاسب",
  },
];
    ...
    >
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center mb-12 sm:mb-20">
          ...
          >
            {isAr ? "تحدي الستين دقيقة" : "The 60-Minute Challenge"}
          </motion.h2>
              ...
              {features.map((feature, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`flex ${(!feature.description && !feature.arabicDescription) ? 'items-center' : 'items-start'} gap-4 sm:gap-6 group transition-all duration-300 ${
                      !isCenter
                        ? isAr
                          ? "lg:mr-12"
                          : "lg:ml-12"
                  ...
                  >
                    ...
                    >
                      <EmojiIcon
                        emoji={feature.emoji}
                        size={isCenter ? 24 : 24}
                        className={`text-white sm:w-[32px] sm:h-[32px] ${isCenter ? "lg:w-[36px] lg:h-[36px]" : ""}`}
                      />
                    </div>
                    <div className={isCenter ? "mt-0.5 sm:mt-1 lg:mt-1.5" : ""}>
                      <h3
                        className={`${isCenter ? "text-lg sm:text-2xl lg:text-xl lg:sm:text-3xl" : "text-lg sm:text-2xl"} font-extrabold text-gray-900 mb-1 sm:mb-2 leading-tight`}
                      >
                        {isAr ? feature.arabicTitle : feature.title}
                      </h3>
                      <p
                        className={`text-gray-500 font-medium leading-relaxed max-w-sm ${isCenter ? "text-sm sm:text-base lg:text-base lg:sm:text-lg" : "text-sm sm:text-base"}`}
                      >
